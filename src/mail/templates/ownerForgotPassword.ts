interface OwnerForgotPasswordData {
  otp: string;
  token: string;
  otpExpiry: Date;
...
export const generateOwnerForgotPasswordEmail = (
  data: OwnerForgotPasswordData,
): string => {
  ...
  const resetUrl = `${process.env.OWNER_FRONTEND_URL}/reset-password?token=${token}`;
