interface UserAddedToPracticeData {
  email: string;
  firstName: string | null;
  practiceName: string;
  role: "ADMIN" | "THERAPIST" | "SUPERVISOR";
  addedAt: Date;
}

export const userAddedToPracticeEmail = (
  data: UserAddedToPracticeData,
): string => {
  const { email, firstName, practiceName, role, addedAt } = data;
  ...
  const formattedDate = new Date(addedAt).toLocaleString("en-US", {
    dateStyle: "medium",
    timeStyle: "short",
  });
...
<head>
  ...
  <style>
    ...
    .box { background:#F0F4EB; padding:24px; border-radius:12px; margin-bottom:25px; }
...
<body>
  <div class="container">
    <div class="header">
      ...
      <h1 style="color:#fff;margin:0;font-size:28px;font-weight:700;">You've Been Added</h1>
    </div>

    <div class="content">
      <h2 style="color:#2C3A2C;font-size:26px;margin:0 0 20px;">Hi ${name},</h2>
      <p>You have been added to a practice on Journi.</p>

      <div class="box">
        <div style="font-size:13px;text-transform:uppercase;letter-spacing:1px;color:#5C665C;margin-bottom:8px;">Practice</div>
        <div style="font-size:22px;font-weight:700;color:#2C3A2C;">${practiceName}</div>
      </div>

      <div class="box">
        <div style="font-size:13px;text-transform:uppercase;letter-spacing:1px;color:#5C665C;margin-bottom:8px;">Assigned Role</div>
        <div style="font-size:22px;font-weight:700;color:#4A7043;">${roleLabel}</div>
      </div>
      ...
      <a href="${process.env.USER_FRONTEND_URL}/login" class="button">
        Login to Your Account
      </a>

      <div style="margin-top:40px;padding:25px;background:#F0F4EB;border-radius:12px;border-left:5px solid #5F7A5A;">
        <div style="font-size:14px;color:#5C665C;">Added on:
          <strong style="color:#2C3A2C;">${formattedDate}</strong>
        </div>
      </div>
    </div>
    ...
    <div style="background:#F9F7F0;padding:30px;text-align:center;font-size:14px;color:#5C665C;border-top:1px solid #E5E9E0;">
      ...
      <a href="mailto:${process.env.SUPPORT_EMAIL}" style="color:#5F7A5A;text-decoration:none;">
